# Django Notes + PostgreSQL Lab 📝

A small but realistic lab project where I built:

- A **Django** “Notes” app (frontend + backend)
...
```text
.
├── docker-compose.yml        # Orchestrates Django + Postgres
└── web
    ├── Dockerfile            # Django app image build instructions
...
3️⃣ (Optional) Create a superuser
...
🗃 Connecting from DBeaver (Host → Container DB)
...
- The id column is auto-generated by PostgreSQL, so we intentionally don’t specify it in the INSERT.

## 🐛 Issues I Faced (And What I Learned)

1️⃣ django.db.utils.ProgrammingError: relation "notes_note" does not exist
...
docker compose run --rm web python manage.py migrate
```
2️⃣ Wrong timestamps (UTC vs IST)
...
USE_TZ = True
```
3️⃣ DBeaver timezone error

- PostgreSQL didn’t like the old alias Asia/Calcutta when connecting via DBeaver.

- Fix: Set the timezone to Asia/Kolkata in DBeaver’s preferences / connection settings.

4️⃣ Inserting into created_at from SQL
...
```
## 📊 Monitoring (Configured on the VM, not in this repo)
